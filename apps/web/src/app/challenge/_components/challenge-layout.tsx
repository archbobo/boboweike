...
export function ChallengeLayout({ left, right }: ChallengeLayoutProps) {
  ...
  return (
    ...
    >
      <div
        className="min-h-[318px] w-full overflow-hidden rounded-l-2xl rounded-r-xl border border-zinc-300 bg-white lg:min-w-[500px] dark:border-zinc-700 dark:bg-zinc-800"
        ref={leftSide}
      >
        {left}
      </div>
      <div className="resizer relative p-2" ref={resizer}>
        <div className="absolute left-1/2 top-1/2 h-1 w-24 -translate-x-1/2 -translate-y-1/2 rounded-full bg-neutral-400 duration-300 group-hover:bg-neutral-600 group-active:bg-emerald-400 group-active:duration-75 lg:h-24 lg:w-1 dark:bg-neutral-700 group-hover:dark:bg-neutral-500" />
      </div>
      <div
        className="flex min-h-[90px] w-full flex-1 flex-col overflow-hidden rounded-l-xl rounded-r-2xl border border-zinc-300 lg:min-w-[500px] dark:border-zinc-700"
        ref={rightSide}
      >
        {right}
      </div>
